...
# 👇 NEW: import preprocessing from separate file
...
if uploaded_file:
    ...
    avg_metrics = (
        filtered_df
        .groupby("success")[["budget", "revenue", "popularity", "runtime", "vote_average"]]
        .mean()
        .reset_index()
    )
    ...
    st.markdown(
        f"**T-Test (Vote Average by Success):** "
        f"t-stat = {t_stat:.2f}, p = {p_val:.4f}"
    )

    contingency = pd.crosstab(filtered_df["main_genre"], filtered_df["success"])
    chi2, p, dof, expected = chi2_contingency(contingency)
    st.markdown(
        f"**Chi-Square (Genre vs Success):** "
    ...
    if submit:
        ...
        if prediction == 1:
            st.success(
                f"🌟 Success predicted with {prediction_proba:.2%} confidence."
            )
        else:
            st.error(
                f"🚨 Unsuccessful predicted with {prediction_proba:.2%} confidence."
            )
